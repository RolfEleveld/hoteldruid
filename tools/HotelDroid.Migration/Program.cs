...
{
    Console.WriteLine("""
        hoteldroid-migrate — Legacy HotelDruid database migration tool

        Usage:
          hoteldroid-migrate --db-type mysql --connection "Server=...;Database=...;User=...;Password=..." [options]
          hoteldroid-migrate --db-type sqlite --connection "Data Source=/path/to/hoteldruid.db" [options]
        ...
        Options:
          ...
          --api-url       base URL of HotelDroid API          (required for import mode)
        ...
        Examples:
          ...
          hoteldroid-migrate --db-type mysql --connection "Server=localhost;Database=hoteldruid;User=root;Password=secret" --output my-export.zip

          # Import from SQLite directly to API:
          hoteldroid-migrate --db-type sqlite --connection "Data Source=hoteldruid.db" --mode import --api-url https://my-server/
        """);
    return 0;
}
...
// ── Run migration ─────────────────────────────────────────────────────────────
Console.WriteLine("HotelDroid Legacy Migration Tool");
