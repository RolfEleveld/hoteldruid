<?php if (!empty($show_rule1_confirmation) && !empty($rule1_confirmation_data)): ?>
<div class="rule-confirmation-box" style="background: linear-gradient(135deg, #fff3cd 0%, #ffeaa7 100%); border-left: 5px solid #ffc107; padding: 20px; margin: 20px 0; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <form accept-charset="utf-8" method="post" action="crearegole.php">
        <div class="linhbox">
            ...
            <input type="hidden" name="mod_idregola1" value="<?php echo htmlspecialchars($rule1_confirmation_data['mod_idregola1']); ?>">
            ...
            <input type="hidden" name="chiudi_app" value="<?php echo htmlspecialchars(fixstr($rule1_confirmation_data['chiudi_app'])); ?>">
            <input type="hidden" name="appartamento" value="<?php echo htmlspecialchars($rule1_confirmation_data['appartamento']); ?>">
            <input type="hidden" name="inizioperiodo" value="<?php echo htmlspecialchars($rule1_confirmation_data['inizioperiodo']); ?>">
            <input type="hidden" name="fineperiodo" value="<?php echo htmlspecialchars($rule1_confirmation_data['fineperiodo']); ?>">
            <input type="hidden" name="motivazione" value="<?php echo htmlspecialchars(fixstr($rule1_confirmation_data['motivazione'])); ?>">
            
            <p style="margin-top: 0;"><em><?php echo mex("Regole esistenti",$pag); ?>:</em></p>
            
            <?php 
            $vecchia_regola = $rule1_confirmation_data['vecchia_regola'];
            $num_vecchia_regola = $rule1_confirmation_data['num_vecchia_regola'];
            $appartamento = $rule1_confirmation_data['appartamento'];
            $idinizioperiodo = $rule1_confirmation_data['idinizioperiodo'];
            $idfineperiodo = $rule1_confirmation_data['idfineperiodo'];
            ...
            ?>
                <div style="margin: 10px 0; padding: 10px; background: rgba(255,255,255,0.5); border-radius: 4px;">
                    ...
                    if ($motivazione2_v == "x") {
                        ...
                        echo mex("Chiedi prima di assegnare", $pag);
                    }
                    ?>
                    <?php echo " " . mex("l'appartamento", 'unit.php') . " " . htmlspecialchars($appartamento) . " " . mex("nel periodo dal", $pag) . " " . htmlspecialchars($datainizio_v) . " " . mex("al", $pag) . " " . htmlspecialchars($datafine_v); ?>
