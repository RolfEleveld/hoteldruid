...
# Taglia dopo il numero di lettere. Per tagliare con numero di byte (quindi possibilmente con meno lettere di $lung) usare par_taglia() in funzioni.php
function tronca_testo ($testo,$ini,$lung=null) {
...
if (strstr($json,"\"$contenuto\":")) { $json = substr(strstr($json,"\"$contenuto\":"),strlen("\"$contenuto\":")); $contenuto = ""; }
else {
if (strstr($json,",$contenuto:")) { $json = substr(strstr($json,",$contenuto:"),strlen(",$contenuto:")); $contenuto = ""; }
elseif (strstr($json,"{"."$contenuto:")) { $json = substr(strstr($json,"{"."$contenuto:"),strlen("{"."$contenuto:")); $contenuto = ""; }
} # fine else if (strstr($json,"\"$contenuto\":"))
if ($non_contenuto and $contenuto) $contenuto = "";
elseif (!$contenuto) {
if ($non_contenuto) $prima_parte = substr($prima_parte,0,(-1 * strlen($json)));
if (substr(trim($json),0,1) == "{") {
if (!$non_contenuto) $prima_parte = substr(trim($json),1);
...
if (!$non_contenuto) $json = substr($prima_parte,0,(-1 * (strlen($json) + 1)));
} # fine if (substr(trim($json),0,1) == "{")
if ($non_contenuto) $json = $prima_parte.$json;
} # fine elseif (!$contenuto)
unset($prima_parte);
} # fine if ($contenuto)
...
function http_chunked_decodif ($str) {
$pos = 0;
$lung = strlen($str);
$risul = null;
$pos_acapo = strpos($str,"\n",1);
$strlungHex = substr($str,$pos,$pos_acapo);
while ($pos < $lung and $strlungHex) {
$hex = strtolower(trim(ltrim($strlungHex,"0")));
if (empty($hex)) $hex = 0;
$dec = hexdec(preg_replace("/[^0-9a-f]/","",$hex));
if ($hex != dechex($dec)) return $str;
$pos = $pos_acapo + 1;
$strlung = hexdec(rtrim($strlungHex,"\r\n"));
$risul .= substr($str, $pos, $strlung);
$pos = strpos($str, "\n", $pos + $strlung) + 1;
$pos_acapo = strpos($str,"\n",($pos + 1));
$strlungHex = substr($str,$pos,($pos_acapo - $pos));
} # fine while ($pos < $lung and $strlungHex)
return $risul;
} # fine function http_chunked_decodif
