...
$tabelle_lock = "";
if (!empty($subtotale_selezionate) and !$pcanc) $altre_tab_lock = array($tableprenota,$tableprenotacanc,$tableperiodi,$tableappartamenti,$tablecontratti,$tablepersonalizza,$tableutenti);
...
$costo_tariffa = (double) fixset($nome_tariffa[1]);
...
echo "$anno_imp - ".$id_periodi_importati[$anno_imp]['primo']." - ".$id_periodi_importati[$anno_imp]['primo_prec']." | ".$id_periodi_importati[$anno_imp]['ultimo']." - ".$id_periodi_importati[$anno_imp]['ultimo_prec']." | $idini - $idfine QUI1 - ";
...
echo "$idini - ".fixset($id_periodi_importati[$anno_imp][$idini])." - ".fixset($id_periodi_importati[$anno_imp][$idfine])." QUI2<br>";
...
echo "$idini - $idfine - $tariffa_chiusa QUI3<br>";
...
$tariffa[1] = (double) fixset($tariffa[1]);
$sconto = risul_query($tutte_prenota,$num1,'sconto',$tableprenota_stat);
if ((double) $sconto > $tariffa[1]) $sconto = $tariffa[1];
...
if ($tasse_perc) {
$tasse = (($tariffa[1] - (double) $sconto) / ($tasse_perc + 100)) * $tasse_perc;
...
for ($numca = 0 ; $numca < $dati_cap['num'] ; $numca++) {
$costo_agg_parziale = (double) calcola_prezzo_totale_costo($dati_cap,$numca,$id_data_inizio,$id_data_fine,$dati_cap[$numca]['settimane'],$dati_cap[$numca]['moltiplica_costo'],$tariffa[1],$tariffe_settimanali_int,((double) $tariffa[1] + $costo_agg_tot - (double) $sconto),$caparra,$numpersone,fixset($dati_cap[$numca]['cat_pers']),0,$costo_escludi_perc);
$costo_agg_tot = (double) $costo_agg_tot + $costo_agg_parziale;
if ($dati_cap[$numca]['escludi_tot_perc'] == "s") $costo_escludi_perc = (double) $costo_escludi_perc + $costo_agg_parziale;
if ($dati_cap[$numca]['tasseperc']) {
