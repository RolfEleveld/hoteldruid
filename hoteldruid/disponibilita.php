...
if ($dati_cat_pers[$num3]['perc'] == "100") $coeff_cat_persone_nt['i'] += (double) $numpersone_corr_cp;
else $coeff_cat_persone_nt['p'] += ((double) $numpersone_corr_cp * ((double) $dati_cat_pers[$num3]['perc'] / 100));
} # fine if (!empty($cat_persone_tariffa_cp[$numtariffa][$num3]) and $numpersone_cp)
else $cat_persone_nr[$numtariffa][$num2][$num3] = "0";
} # fine for $num3
$costo_tariffap_settimana_alt = (round(((double) $costo_tariffap_sett * $coeff_cat_persone_nt['p']) / $dati_cat_pers['arrotond']) * $dati_cat_pers['arrotond']) + ((double) $costo_tariffap_sett * $coeff_cat_persone_nt['i']);
$costo_tariffa_settimana_tot_alt = $costo_tariffa_settimana + $costo_tariffap_settimana_alt;
...
$prezzo_costo[$numca][0] = (double) calcola_prezzo_totale_costo($dati_ca,$numca,$idinizioperiodo,$idfineperiodo,$settimane_costo[$numca],$moltiplica_costo_corr,$costo_tariffa_corr,$tariffesettimanali_corr,($costo_tariffa_corr + $costi_agg_tot),$caparra_corr,$numpersone_corr,$dati_cat_pers,$catpers_ca,$costo_escludi_perc,1);
